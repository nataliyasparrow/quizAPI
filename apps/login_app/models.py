...
class TripManager(models.Manager):
    ...
    #     errors = {}
    #     if postData['msg_user_id'] != postData['user_id']:
    #         errors["delete_message"] = "You can delete your own messages only!"
    #     else:
    #         created_at = Message.objects.get(id=postData['msg_id']).created_at
    #         current_time = datetime.datetime.utcnow().replace(tzinfo=pytz.UTC)
    #         if created_at + datetime.timedelta( minutes=30 ) < current_time: 
    #             errors["delete_message"] = "You can delete your new messages only!"
        # return errors
...
#     def basic_validator(self, postData):
#         errors = {}
#         if len(postData['post_comment']) < 1:
#             errors["post_comment"] = "Comment can't be blank"
#         return errors
...
#     def __repr__(self):
#         return f"<Comment object: {self.user.name} ({self.id})>"
